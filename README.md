...
> - **Icon overrides.** The `IconOverrides` setting (XML in `HKCU\Software\EarTrumpet` for the unpackaged build) replaces an app's icon, keyed like the hidden list: an executable name, or `#systemsounds` for System Sounds. Example: `#systemsounds` with `%SystemRoot%\System32\imageres.dll,-5308` draws System Sounds with the four-pane Windows glyph. There is no UI for this yet.
>
