...
>
> - **Hide apps.** Right-click an app in the flyout or the full mixer and press *Hide* (the crossed-out eye). Hidden apps are keyed by executable name (for example `steam.exe`), so they stay hidden across restarts. Only the row is hidden; the app's sound keeps playing.
> - **Apps settings page.** The gear at the top right of the flyout opens *Settings > Apps*, which lists the apps in the mixer (hide them or change their icon), the hidden apps (*Show* brings one back, *Hide another program…* adds any `.exe`), and the custom icons.
...
> - **Custom icons.** *Change icon…* opens the Windows icon picker (`.ico`, `.exe` or `.dll`); *Original icon* goes back to the app's own. Stored in the `IconOverrides` setting, keyed like the hidden list (`#systemsounds` for System Sounds).
> - **Windows logo for System Sounds.** Out of the box, System Sounds is drawn with the four-pane Windows glyph from `imageres.dll` instead of the speaker. It is an ordinary custom icon: change it or press *Original icon* in *Settings > Apps*.
>
