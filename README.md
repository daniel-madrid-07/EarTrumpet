...
> - **Custom icons.** *Change icon…* opens the Windows icon picker (`.ico`, `.exe` or `.dll`); *Original icon* goes back to the app's own. Stored in the `IconOverrides` setting, keyed like the hidden list, with `#systemsounds` for System Sounds (for example `%SystemRoot%\System32\imageres.dll,-5308`, the four-pane Windows glyph).
>
